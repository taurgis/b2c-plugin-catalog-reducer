...
import {SelectorConfig, XmlNode} from './types';

export const CACHE_DIR_NAME = '.catalog-reducer-cache';
 ...
 */
const hashFileContents = (inputFilename: string): Promise<string> => new Promise((resolve, reject) => {
 ...
 */
export const computeCacheKey = async (inputFilename: string, selectorConfig: SelectorConfig): Promise<string | null> => {
  try {
...
const resolveCachePath = (inputFilename: string, cacheKey: string): string => {
 ...
 */
export const readCachedSelection = async (inputFilename: string, cacheKey: string | null): Promise<XmlNode[] | null> => {
  if (!cacheKey) {
    return null;
  }
 ...
 */
export const writeCachedSelection = async (inputFilename: string, cacheKey: string | null, selectedProducts: XmlNode[]): Promise<void> => {
  if (!cacheKey) {
    return;
  }
 ...
 */
export const getOrCompute = async (
  inputFilename: string,
  selectorConfig: SelectorConfig,
  {compute, persist = true}: {compute: () => Promise<XmlNode[]>; persist?: boolean}
): Promise<{selection: XmlNode[]; cacheHit: boolean}> => {

...
const hasPreferredProductIds = selectorConfig => {
    return Array.isArray(selectorConfig.productIds) && selectorConfig.productIds.length > 0;
};
...
const hasMasterSelection = selectorConfig => Number(selectorConfig.master) > 0;
...
const shouldUseSinglePassSelector = selectorConfig => {
    return selectorConfig
        && selectorConfig.singlePass === true
        && !hasCustomAttributeSelection(selectorConfig);
};
...
const hasAnySelectionTarget = selectorConfig => {
    return Number(selectorConfig.total) > 0
...
exports.parse = async function (inputFilename, outputFilename, selectorConfig) {
    ...
    if (shouldUseSinglePassSelector(selectorConfig)) {
        const { selectProductsSinglePass } = require('./singlePassSelector');
        const singlePassResult = await selectProductsSinglePass(inputFilename, selectorConfig);

        selectedProducts = singlePassResult.selection;
    } else {
        const FilterManager = require('./filterManager');
        const masterFilter = require('./filters/masterFilter');
        const preferredMasterProductFilter = require('./filters/preferredMasterProductsFilter');
        ...
        if (hasPreferredProductIds(selectorConfig)) {
            filterManager.registerFilter(preferredMasterProductFilter);
        }
        ...
        selectedProducts = filterManager.getSelection();
    }

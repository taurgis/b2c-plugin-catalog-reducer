...
description: Create and extend Salesforce B2C Developer Tooling CLI plugins (commands, hooks, middleware, scaffolds). Use when building the target B2C plugin for this repo, registering hooks, or troubleshooting plugin loading during the catalog reducer migration.
---

# B2C CLI Extending

Use this skill when building the target B2C CLI plugin for this repository or integrating with the B2C Developer Tooling SDK during the catalog reducer migration.
...
- Map reducer capabilities currently implemented under `tmp/` into B2C CLI command or hook boundaries.
...
- Describing a final command namespace or package shape before the migration plan is confirmed.

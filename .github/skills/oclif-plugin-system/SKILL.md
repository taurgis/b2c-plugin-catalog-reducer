...
description: Build, configure, and troubleshoot oclif CLI plugins (commands, hooks, plugin bundles). Use when structuring the future catalog reducer plugin, wiring package.json, or linking plugins for local development.
---

# Oclif Plugin System

Use this skill when you need to create or extend oclif plugins, register commands or hooks, or debug plugin loading. In this repository it is most relevant when turning the current reducer implementation under `tmp/` into a plugin-oriented structure.
...
- Migration note: preserve reducer behavior first, then decide whether logic belongs in commands, shared services, or hooks.
...
- Designing command structure before confirming which reducer behaviors stay in the current CLI versus the future plugin.
